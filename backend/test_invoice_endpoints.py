...
# Prevent OCI authentication and OCI client initialization during
# CI/local endpoint-test import. These tests do not call OCI.
with patch(
    "services.oci_auth.get_oci_config",
    return_value=(
        {"region": "ci-test"},
        None,
        "ci-test-compartment",
...
assert res.status_code == 400, (
    f"Expected 400 for non-PDF, got {res.status_code}"
)

print(
    "[PASS] POST /api/invoices/upload "
    "rejects non-PDF (HTTP 400)"
)
...
assert res.status_code == 422, (
    f"Expected 422 for missing file, got {res.status_code}"
)

print(
    "[PASS] POST /api/invoices/upload "
    "rejects missing file (HTTP 422)"
)


# 4. Status 404 for unknown processing ID
res = client.get(
    "/api/invoices/invalid-uuid-12345/status"
)

assert res.status_code == 404, (
    f"Expected 404 for unknown ID, got {res.status_code}"
)

print(
    "[PASS] GET /api/invoices/{id}/status "
...
print(
    "[PASS] GET /api/invoices/{id}/status "
    "returns active state (UPLOADED, 10%)"
)
...
print(
    "[PASS] Real-time status update verified "
    "(PROCESSING, 55%, stage=OCI_DOCUMENT_UNDERSTANDING)"
)
    ...
    "line_items": [
        {
            "line_number": 1,
            "description": (
                "Referral : Payouts for Business Loan "
                "the Month of Oct, 2025"
            ),
            "item_number": "997159",
            "quantity": 0,
...
assert (
    result_json["invoice"]["vendor_name"]
    == "AADIFIDELIS SOLUTIONS PRIVATE LIMITED"
)

assert (
    result_json["invoice"]["invoice_number"]
    == "1025/BL/DL/0889"
)
...
assert (
    result_json["line_items"][0]["line_amount"]
    == 1500142
)

print(
    "[PASS] GET /api/invoices/{id}/result "
    "returns normalized invoice and line items (HTTP 200)"
)
